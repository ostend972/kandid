...
export function computeLegitimacyScore(
  ...
  repostCount?: number,
  livenessResult?: 'active' | 'expired' | 'uncertain',
): LegitimacyResult {
  ...
  // --- Liveness signal ---
  if (livenessResult === 'active') {
    signals.push({ signal: 'liveness', finding: 'URL confirmed active', weight: 10 });
    score += 10;
  } else if (livenessResult === 'expired') {
    signals.push({ signal: 'liveness', finding: 'URL confirmed expired', weight: -30 });
    score -= 30;
  } else if (livenessResult === 'uncertain') {
    signals.push({ signal: 'liveness', finding: 'URL liveness uncertain', weight: -10 });
    score -= 10;
  }
  ...
  let tier: LegitimacyTier = score >= 60 ? 'high' : score >= 30 ? 'caution' : 'suspicious';

  if (livenessResult === 'expired') {
    tier = 'suspicious';
  }
